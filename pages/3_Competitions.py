"""Manage competitions, per-competition team assignments and SDC nominees."""
...
with tab_sched:
    ...
    # Quick KPI strip
    dated = df[df["date"].fillna("") != ""]
    kpi_row([
        {"icon": "📅", "label": "Competitions",   "value": len(df)},
        {"icon": "📍", "label": "With date set",  "value": len(dated)},
        {"icon": "🏟", "label": "Venues listed",
         "value": int(df["venue"].fillna("").ne("").sum())},
    ])

    section_label("Competition schedule")
...
with tab_teams:
    st.caption(
        "Teams A–I are assigned **fresh per competition**. Past comps keep "
        "their assignments — changing a future comp's teams won't affect "
    ...
    club_filter = st.multiselect("Filter by club", CLUBS, default=[])
    view = anglers.copy()
    if club_filter:
        view = view[view["club"].isin(club_filter)]
    view["Angler"] = (view["first_name"].fillna("") + " "
                      + view["surname"].fillna(""))
    view = view.merge(
        ta_comp[["wp_no", "sub_team"]].rename(columns={"sub_team": "team"}),
        on="wp_no", how="left")
    view["team"] = view["team"].fillna("")
    view = (view[["wp_no", "Angler", "club", "team"]]
            .sort_values(["club", "Angler"])
            .reset_index(drop=True))

    TEAM_LIMIT = 8
    edited = st.data_editor(
        view, use_container_width=True, hide_index=True,
        key=f"team_edit_{comp}",
        column_config={
            ...
            "team":   st.column_config.SelectboxColumn(
                          ...
                          help=f"Blank = not selected. Max {TEAM_LIMIT} per team."),
        },
    )

    # Team size indicators
    counts = (edited[edited["team"] != ""].groupby("team").size()
              .reindex(SUB_TEAMS, fill_value=0))
    over   = [t for t in SUB_TEAMS if counts[t] > TEAM_LIMIT]
    section_label(f"Team sizes for {comp} (max {TEAM_LIMIT})")
    cs = st.columns(len(SUB_TEAMS))
    for i, t in enumerate(SUB_TEAMS):
        n     = int(counts[t])
        delta = f"+{n - TEAM_LIMIT}" if n > TEAM_LIMIT else None
        cs[i].metric(f"Team {t}", n, delta=delta,
                     delta_color="inverse" if delta else "off")

    if over:
        st.error(f"Team(s) {', '.join(over)} exceed {TEAM_LIMIT} anglers. "
                 "Reduce before saving.")

    c1, c2, _ = st.columns([1, 1, 4])
    if c1.button("💾 Save teams", type="primary",
                 disabled=bool(over), use_container_width=True):
        ...
        st.rerun()
    if c2.button("🗑 Clear teams", use_container_width=True):
        keep_other = ta_all[ta_all["comp_id"] != comp]
...
with tab_nom:
    st.caption(
        "**Sir Drummond Chapman Trophy** — each club nominates **4 anglers** "
    ...
    if not jan_comp:
        st.warning("No competition with a January date found. "
                   "Set comp dates on the Schedule tab (format YYYY-MM-DD).")
    else:
        st.info(f"Target competition: **{jan_comp}**")
        anglers = load_anglers()
        if anglers.empty:
            ...
            if club_anglers.empty:
                ...
                club_anglers["Angler"] = (
                    club_anglers["first_name"].fillna("") + " "
                ...
                if n_picked > 4:
                    ...
                    st.caption(f"Selected: **{n_picked}** / 4")

                if st.button("💾 Save nominees",
                    ...
                    save_trophy_nominees(
                        pd.concat([keep, new_rows], ignore_index=True))
                    st.success(f"Saved {n_picked} SDC nominee(s) for {club_pick}.")
                    st.rerun()

            divider_label("All current SDC nominees")
            if sdc.empty:
                st.info("No nominees set yet.")
            else:
                pretty = sdc.merge(
                    anglers.assign(
                        Angler=lambda d: (d["first_name"].fillna("") + " "
                                          + d["surname"].fillna("")).str.strip()
                    )[["wp_no", "Angler"]],
                    on="wp_no", how="left")
                pretty = (pretty[["club", "wp_no", "Angler"]]
